...
import icon1 from "@/assets/images/live_icon.png";
import icon2 from "@/assets/images/live/eye.png";
import icon3 from "@/assets/images/live/loading.png";
...
import icon11 from "@/assets/images/live/background-h5.png";
  ...
  if (loginData.value.data.code === 200) {
    ...
    if (process.client) {
      ...
      const handleUserPublished = async (user, mediaType) => {
        ...
        if (mediaType === "audio") {
          //const audioTrackLive = user.audioTrack;
          audioTrack.value = user.audioTrack;
          // 自动播放音频
          //audioTrackLive.play();
        } else {
          const videoTrackLive = user.videoTrack;
          videoTrack.value = videoTrackLive;
          if (window.innerWidth > 762) {
            videoTrackLive.play(videoRef1.value);
          } else {
            videoTrackLive.play(videoRef.value);
            icon11Imgage.value = ''
          }
          // 自动播放视频
          isPlay.value = true
      ...
      client.on("connection-state-change",(e,data) => {
        if (e.state === 'CLOSED') {
          isPlaying.value = false
          icon11Imgage.value = icon11;
        }
      });
      client.on("error", (e,data) => {
        //isPlaying.value = false
      });
...
// 将MD5哈希值转为大写
  ...
  if (process.client) {
    ...
    audioTrack.value.stop()
    if (videoTrack.value && videoTrack.value.leave()) {
    }
...
const getChatmsg = async () => {
  if (process.client) {
    ...
    const loginResp = await LavieSDK.accountLoginHandle(userInfo.value,'sdfasdfsdf232rfsdfsd20022');
    // const micResp = await LavieSDK.queryOnlineRoomUserList(route.params.id, 1);
     const eventId = LavieSDK.addMsgListener(LavieSDK.MESSAGE_TYPE.ROOM_MESSAGE,
         (e: MessageData) => {
...
// 将MD5哈希值转为大写
  ...
  if (loginData.value.data.code === 200) {
    //userList.value = loginData.value.data.items;
    ...
    <section class=" hidden lg:flex lg">
      ...
      <div class="lg:p-6 lg:px-4 py-2 px-4 items-center flex max-w-7xl header-tag" style="width: 100%; height: 100%; padding-top: 16px; padding-bottom: 16px; margin: 100px auto auto auto;">
        <div style="justify-content: flex-start; align-items: center; gap: 4px; display: flex">
          ...
          <div style="justify-content: flex-start; align-items: center; display: flex">
            <div style="color: rgba(0, 0, 0, 0.90); font-size: 14px; font-weight: normal; font-weight: 400; line-height: 20px; word-wrap: break-word"><nuxt-link :to="'/liveDetail/' + id">Live detail</nuxt-link></div>
          </div>
        </div>
...
<!--                <div id="svga" style="width:100px;height: 100px;">1111</div>-->

        <div style="border-radius: 12px; overflow: hidden; flex-direction: column; justify-content: flex-start; align-items: flex-start; display: inline-flex">
          ...
          <div style="width: 926px; height: 60px; position: relative; background: white">
            ...
            <div style="left: 64px; top: 14px; position: absolute; justify-content: flex-start; align-items: center; gap: 9px; display: inline-flex">
              <div style="color: #09090B; font-size: 16px; font-family: Figtree; font-weight: 600; word-wrap: break-word">{{dataInfo.userName}}</div>
              <div style="padding-left: 5.33px; padding-right: 5.33px; padding-top: 1.33px; padding-bottom: 1.33px; border-radius: 6666px; justify-content: flex-start; align-items: center; gap: 1.33px; display: flex">
                <div style="width: 100%; position: relative">
                  <img style="width: 41.6px;" :src="icon1" />
                </div>
              </div>
            </div>
...
<!--              </div>-->
<!--              <div style="text-align: right; color: #D4D4D8; font-size: 12px; font-weight: normal; font-weight: 500; line-height: 16px; word-wrap: break-word">1122</div>-->
<!--            </div>-->
          </div>
          <div style="width: 926px; height: 521px; position: relative">

            <div  style="width: 926.22px; height: 521px; left: 0px; top: 0px; position: absolute; background: #FFF1F2"></div>
            <img   style="width: 926px; height: 521px; left: 0px; top: 0px; position: absolute" :src="icon3" />
            <div    style="width: 926px; height: 521px; left: 0px; top: 0px; position: absolute; background: rgba(0, 0, 0, 0.60); backdrop-filter: blur(21px)"></div>
            <div id="svgaPc" style="position: absolute;top:20%;left:25%;z-index: 999;width: 400px;height: 300px"></div>
<!--            <div style=" left: 852px; top: 12px; position: absolute;  border-radius: 6666px; justify-content: flex-start; align-items: center; gap: 1.33px; display: inline-flex">-->
<!--              <div style="width: 100%; height: 10.67px; position: relative">-->
<!--                <img style="width: 41.6px;height: 16.6px;" :src="icon1" />-->
<!--              </div>-->
<!--            </div>-->
            <div style="width: 56px; height: 56px; left: 435px; top: 232px; position: absolute" v-if="!isPlay">
              <div style="width: 46.67px; height: 46.67px; left: 4.67px; top: 51.33px; position: absolute; transform: rotate(-90deg); transform-origin: 0 0; ">
                <img class="rotating-image" style="width: 46.67px; height: 46.67px;" :src="icon4" />
              </div>
            </div>
            <div style=" left: 370px; top: 232px; position: absolute" v-if="!isPlaying">
              <div style="width: 100%; height: 100%; flex-direction: column; justify-content: flex-start; align-items: center; gap: 8px; display: inline-flex">
                <img style="width: 56px; height: 56px; border-radius: 9999px; border: 1px rgba(255, 255, 255, 0.50) solid" :src="dataInfo.avatarUrl" />
              </div>
              <div style="text-align: center; color: white; font-size: 14px; font-family: Figtree; font-weight: 600; word-wrap: break-word">The anchor has left the room</div>
            </div>
            <div ref="videoRef1" style="height: 100%;width: 50%;position:absolute;left:25%;">
            </div>
          ...
          <div style="width: 926px; height: 110px;margin-right: 70px; padding-top: 5px; padding-bottom: 8px; padding-left: 290px; padding-right: 8px; background: white; justify-content: flex-end; align-items: center; display: inline-flex">
                ...
                <div style="width: 70px;height: 62px; padding-left: 2px; padding-right: 2px;   background: #FAFAFA; border-radius: 6px; flex-direction: column; justify-content: flex-start; align-items: center; display: inline-flex">
                  <ClientOnly>
                      <el-popover
                          ...
                          :persistent=true
                          :teleported=true
                          popper-style="border-radius: 20px; background-color: #232136;"
                      >
...
<!--      <div id="svga" style="background-color: #000000; width: 500px; height: 500px; margin: auto"></div>-->
      <div ref="videoRef"  :style="{ backgroundImage: 'url(' + icon11Imgage + ')' }">
        <div style="width: 100%;z-index:999;  background:rgba(255, 255, 255, 0.12)">
          <div style="padding: 2px; left: 12px; top: 7px; position: absolute; border-top-left-radius: 30px; border-top-right-radius: 30px; justify-content: flex-start; align-items: center; gap: 11px; display: inline-flex">
            <div style="justify-content: flex-start; align-items: flex-start; gap: 6px; display: flex">
              ...
              <div style="flex-direction: column; justify-content: flex-start; align-items: flex-start; display: inline-flex">
                <div style="justify-content: flex-start; align-items: center; gap: 4px; display: inline-flex">
                  <div style="color: rgba(255, 255, 255, 0.83); font-size: 14px; font-family: Figtree; font-weight: 600; max-width: 50%; word-wrap: break-word; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                    {{ dataInfo.title }}
                  </div>
                  <div style="padding-left: 4.26px; padding-right: 4.26px; padding-top: 1.07px; padding-bottom: 1.07px; background: #FF1953; border-radius: 5328.82px; justify-content: flex-start; align-items: center; gap: 1.07px; display: flex">
...
<!--        <div ref="videoRef" style="width: 100%; margin-bottom:20%;height: 100%;">-->

<!--        </div>-->
        <div id="svga" style="width: 100%;top:20%;position: relative">

        </div>

        <div style=" left: 40%; top: 40%; position: absolute" v-if="!isPlay">
          <div style="width: 46.67px; height: 46.67px; left: 4.67px; top: 51.33px; position: absolute; transform: rotate(-90deg); transform-origin: 0 0; ">
            <img class="rotating-image" style="width: 46.67px; height: 46.67px;" :src="icon4" />
          </div>
        </div>
        <div style=" left: 23%; top: 40%; position: absolute" v-if="!isPlaying">
          <div style="width: 100%; height: 100%; flex-direction: column; justify-content: flex-start; align-items: center; gap: 8px; display: inline-flex">
            <img style="width: 56px; height: 56px; border-radius: 9999px; border: 1px rgba(255, 255, 255, 0.50) solid" :src="dataInfo.avatarUrl" />
          </div>
          <div style="text-align: center; color: white; font-size: 14px; font-family: Figtree; font-weight: 600; word-wrap: break-word">The anchor has left the room</div>
        ...
        <div style="width: 100%; height: 100%; bottom:0%;left: 16px;z-index:999;position:absolute;top: 60%; flex-direction: column; justify-content: flex-start; align-items: flex-start; gap: 8px; display: inline-flex">
          ...
          <div ref="chatMsgRef" id="chatMsg1" style="z-index: 999; height: 200px;overflow-y: auto;border: 0px solid #ccc;padding: 10px;" >
            <div v-for="(slide,id) in chatMsgInfo" style="align-self: stretch; justify-content: flex-start; align-items: flex-start; gap: 4px; display: inline-flex">
              <div style="justify-content: flex-start; align-items: flex-start; gap: 2px; display: flex">
        ...
        <div v-if="!voicePlay" @click="play" style="cursor: pointer;z-index: 999; position: fixed;left: 70%;top:85%;">
        ...
        <div v-if="voicePlay" @click="noPlay" style="cursor: pointer;z-index: 999; position: fixed;left: 70%;top:85%;">
          <svg width="79" height="32" viewBox="0 0 79 32" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="79" height="32" rx="16" fill="white" fill-opacity="0.1"/>
            <path d="M10.003 15.7155C10.0403 13.8733 10.059 12.9522 10.6709 12.1636C10.7826 12.0197 10.9465 11.8487 11.0839 11.7327C11.8364 11.0974 12.8299 11.0974 14.8171 11.0974C15.527 11.0974 15.882 11.0974 16.2204 11.0045C16.2907 10.9852 16.3602 10.963 16.4289 10.9378C16.7594 10.8167 17.0557 10.6084 17.6485 10.1916C19.9872 8.54738 21.1565 7.72527 22.138 8.08241C22.3261 8.15088 22.5083 8.24972 22.671 8.37162C23.5194 9.00744 23.5839 10.4868 23.7128 13.4454C23.7606 14.5409 23.7931 15.4785 23.7931 16C23.7931 16.5215 23.7606 17.4591 23.7128 18.5546C23.5839 21.5132 23.5194 22.9926 22.671 23.6284C22.5083 23.7503 22.3261 23.8491 22.138 23.9176C21.1565 24.2747 19.9872 23.4526 17.6485 21.8084C17.0557 21.3916 16.7594 21.1833 16.4289 21.0622C16.3602 21.037 16.2907 21.0148 16.2204 20.9955C15.882 20.9026 15.527 20.9026 14.8171 20.9026C12.8299 20.9026 11.8364 20.9026 11.0839 20.2673C10.9465 20.1513 10.7826 19.9803 10.6709 19.8364C10.059 19.0478 10.0403 18.1267 10.003 16.2845C10.001 16.1878 10 16.0928 10 16C10 15.9072 10.001 15.8122 10.003 15.7155Z" fill="white"/>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M27.4895 9.55219C27.7821 9.29218 28.217 9.33434 28.4608 9.64635L27.931 10.1171C28.4608 9.64635 28.4606 9.64602 28.4608 9.64635L28.4619 9.6477L28.4631 9.64921L28.4658 9.65275L28.4727 9.66184C28.4779 9.6688 28.4844 9.67756 28.4921 9.68814C28.5075 9.70929 28.5275 9.73772 28.5515 9.77358C28.5995 9.84529 28.6635 9.94667 28.7379 10.0789C28.8868 10.3435 29.077 10.7309 29.2644 11.2504C29.6397 12.2911 30 13.8514 30 16.0002C30 18.1491 29.6397 19.7094 29.2644 20.7501C29.077 21.2695 28.8868 21.657 28.7379 21.9216C28.6635 22.0538 28.5995 22.1552 28.5515 22.2269C28.5275 22.2627 28.5075 22.2912 28.4921 22.3123C28.4844 22.3229 28.4779 22.3317 28.4727 22.3386L28.4658 22.3477L28.4631 22.3513L28.4619 22.3528C28.4616 22.3531 28.4608 22.3541 27.931 21.8833L28.4608 22.3541C28.217 22.6661 27.7821 22.7083 27.4895 22.4483C27.1983 22.1895 27.1578 21.729 27.3977 21.417C27.3983 21.4163 27.3994 21.4148 27.4009 21.4127C27.4058 21.406 27.4154 21.3925 27.4291 21.372C27.4565 21.3311 27.5003 21.2625 27.5552 21.1649C27.6649 20.9698 27.8195 20.6587 27.977 20.2221C28.2913 19.3508 28.6207 17.9695 28.6207 16.0002C28.6207 14.0309 28.2913 12.6497 27.977 11.7784C27.8195 11.3418 27.6649 11.0307 27.5552 10.8356C27.5003 10.738 27.4565 10.6693 27.4291 10.6284C27.4154 10.608 27.4058 10.5945 27.4009 10.5878C27.3994 10.5856 27.3983 10.5842 27.3977 10.5834C27.3977 10.5834 27.3977 10.5834 27.3977 10.5834" fill="white"/>
        ...
        <div class="gift-info2">
          ...
          <div class="box" style="background-color: #F59E0B">
            ...
            <nuxt-link to="/recharge.html"><p>Recharge ></p></nuxt-link>
          </div>
        </div>
      </div>
      ...
      <div class="container">
        ...
        <div class="input-container">
          <div class="input-wrapper">
            <input v-for="(digit, index) in digits" inputmode="numeric"  @keydown="onKeyDown(index, $event)" pattern="[0-9]*" :key="index" v-model="digits[index]" @input="onDigitInput(index)" maxlength="1" class="digit-input" />
          </div>
        </div>
...
.section {
  section {
    ...
    &.md {
      ...
      .rotating-image {
        animation: rotate 2 linear infinite; /* 5s表示动画时长，linear表示线性动画，infinite表示无限循环 */
      }
      .gift-info2 {
        display: flex;
        //justify-content: space-between;
